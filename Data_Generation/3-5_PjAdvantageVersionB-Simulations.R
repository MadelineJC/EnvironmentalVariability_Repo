#### DOC STRING ===============================================================
# This script runs (1) environmentally and (2) demographically and environmentally 
# stochastic simulations in the low- and high-average temperature environments 
# under fourth thermal scenario (version B). 
...
# and load the following files: 5-E_LT_PjAd_vB_PV.rds, 10-E_HT_PjAd_vB_PV.rds, 
# 5-DE_LT_PjAd_vB_PV.rds, 10-DE_HT_PjAd_vB_PV.rds.
#### END DOC STRING============================================================
...
#### Simulations ####
#### ... Environmentally stochastic, LATE, Pj-Advantage, Version B ####
...
saveRDS(List3, file = "Generated_Outputs/E_LT_PjAd_vB.rds")
  ...
  for(j in 1:length(temp_list)){ # Loops over intro times
    ...
    for (k in 1:length(temp_list[[1]])){ # Loops over SDs
      ...
      for (m in Pis){
        ...
        # temp_df %>% mutate_if(is.numeric, round, digits = 0)
...
saveRDS(List5, file = "Generated_Outputs/E_LT_PjAd_vB_PV.rds")

#### ... Demographically and environmentally stochastic, LATE, Pj-Advantage, Version B ####
MTE_TPC_Pj <- function(temp){
  lambda_knot <- 1.5; Tknot <- 13+K; Ez <- 0.65; EzL <- (-6.2); EzH <- 6.2; TzL <- (-12)+K; TzH <- 20.6+K; k <- 8.62*10^(-5)
  r <- lambda_knot*
    (exp(-(Ez/k)*((1/temp) - (1/Tknot))))*
...
saveRDS(List3, file = "Generated_Outputs/DE_LT_PjAd_vB.rds")
...
saveRDS(List5, file = "Generated_Outputs/DE_LT_PjAd_vB_PV.rds")

#### ... Temp Sampling Objects and Iterators ####
L <- 499
ar <- 0.25
K <- 273.15
range <- 40
...
#### ... Environmentally stochastic, HATE, Pj-Advantage, Version B ####
MTE_TPC_Pj <- function(temp){
...
saveRDS(List3, file = "Generated_Outputs/E_HT_PjAd_vB.rds")
...
saveRDS(List5, file = "Generated_Outputs/E_HT_PjAd_vB_PV.rds")

#### ... Demographically and environmentally stochastic, HATE, Pj-Advantage, Version B ####
MTE_TPC_Pj <- function(temp){
  lambda_knot <- 0.68; Tknot <- 11.3+K; Ez <- 0.65; EzL <- (-6.2); EzH <- 6.2; TzL <- (-10)+K; TzH <- 28+K; k <- 8.62*10^(-5)
  r <- lambda_knot*
    (exp(-(Ez/k)*((1/temp) - (1/Tknot))))*
...
saveRDS(List3, file = "Generated_Outputs/DE_HT_PjAd_vB.rds")
...
saveRDS(List5, file = "Generated_Outputs/DE_HT_PjAd_vB_PV.rds")
